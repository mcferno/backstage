	...
	<div class="span12">
		...
		<?php
			...
			if($image) {
				...
				echo $this->Html->link('<i class="icon icon-chevron-left"></i> Return to Video', array('action' => 'view', $video['Video']['id']), array('class' => 'btn', 'escape' => false));
			} else {
	...
	<div class="span12">
		...
		<fieldset>
			...
			<?php
				...
				echo '&nbsp;';
				echo $this->Html->link('<i class="icon icon-ban-circle"></i> Cancel', array('action' => 'view', $video['Video']['id']), array('class' => 'btn btn-large', 'escape' => false));
				if($image) {
					echo '&nbsp;';
					echo $this->Html->link('<i class="icon-white icon-pencil"></i> Re-Crop Existing', array('action' => 'image', $video['Video']['id'], 'mode' => 'crop'), array('class' => 'btn btn-info btn-large', 'escape' => false));
				} 
			?>
			</div>
		</fieldset>
